EESchema Schematic File Version 4
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "Mojave MCU Core"
Date ""
Rev "A"
Comp "Paradigm Hyperloop"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 "JR"
$EndDescr
$Comp
L MCU_ST_STM32F3:STM32F303R6Tx U?
U 1 1 5ECBEAF2
P 9700 3650
F 0 "U?" H 9700 3750 50  0000 C CNN
F 1 "STM32F303R6Tx" H 9700 3650 50  0000 C CNN
F 2 "Package_QFP:LQFP-64_10x10mm_P0.5mm" H 9100 1950 50  0001 R CNN
F 3 "http://www.st.com/st-web-ui/static/active/en/resource/technical/document/datasheet/DM00092070.pdf" H 9700 3650 50  0001 C CNN
	1    9700 3650
	1    0    0    -1  
$EndComp
Text Notes 6950 5500 0    50   ~ 0
NOTE: WE'RE USING THE STM32F303Rx SERIES,\nANY CHIPS IN THIS FAMILY *SHOULD* BE PIN\nCOMPATIBLE (CHECK BEFORE TRYING THEM\nOF COURSE), IF WE NEED MORE FLASH OR\nRAM OR SOMETHING
$Comp
L power:GND #PWR?
U 1 1 5ECC2F17
P 1450 1050
F 0 "#PWR?" H 1450 800 50  0001 C CNN
F 1 "GND" H 1455 877 50  0000 C CNN
F 2 "" H 1450 1050 50  0001 C CNN
F 3 "" H 1450 1050 50  0001 C CNN
	1    1450 1050
	1    0    0    -1  
$EndComp
$Comp
L Device:D_Schottky D?
U 1 1 5ECC49CA
P 1700 900
F 0 "D?" H 1700 684 50  0000 C CNN
F 1 "D_Schottky" H 1700 775 50  0000 C CNN
F 2 "" H 1700 900 50  0001 C CNN
F 3 "~" H 1700 900 50  0001 C CNN
	1    1700 900 
	-1   0    0    1   
$EndComp
Text Label 1400 900  0    50   ~ 0
VIN
Text Label 1850 900  0    50   ~ 0
VIN_RP
Wire Wire Line
	1850 900  2050 900 
$Comp
L Device:Fuse F?
U 1 1 5ECC6BCB
P 2200 900
F 0 "F?" V 2003 900 50  0000 C CNN
F 1 "Fuse" V 2094 900 50  0000 C CNN
F 2 "" V 2130 900 50  0001 C CNN
F 3 "~" H 2200 900 50  0001 C CNN
	1    2200 900 
	0    1    1    0   
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J?
U 1 1 5ECC7BDA
P 1200 1000
F 0 "J?" H 1300 900 50  0000 C CNN
F 1 "Conn_01x02" H 1500 1000 50  0000 C CNN
F 2 "" H 1200 1000 50  0001 C CNN
F 3 "~" H 1200 1000 50  0001 C CNN
	1    1200 1000
	-1   0    0    1   
$EndComp
Wire Wire Line
	1400 900  1550 900 
Wire Wire Line
	1450 1050 1450 1000
Wire Wire Line
	1450 1000 1400 1000
$Comp
L Regulator_Switching:R-78E5.0-1.0 U?
U 1 1 5ECCBD82
P 2900 900
F 0 "U?" H 2900 1142 50  0000 C CNN
F 1 "R-78E5.0-1.0" H 2900 1051 50  0000 C CNN
F 2 "Converter_DCDC:Converter_DCDC_RECOM_R-78E-0.5_THT" H 2950 650 50  0001 L CIN
F 3 "https://www.recom-power.com/pdf/Innoline/R-78Exx-1.0.pdf" H 2900 900 50  0001 C CNN
	1    2900 900 
	1    0    0    -1  
$EndComp
Wire Wire Line
	2600 900  2450 900 
$Comp
L Device:C C?
U 1 1 5ECCF3A9
P 2450 1100
F 0 "C?" H 2565 1146 50  0000 L CNN
F 1 "10u" H 2565 1055 50  0000 L CNN
F 2 "" H 2488 950 50  0001 C CNN
F 3 "~" H 2450 1100 50  0001 C CNN
	1    2450 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	2450 950  2450 900 
Connection ~ 2450 900 
Wire Wire Line
	2450 900  2350 900 
$Comp
L Device:C C?
U 1 1 5ECD3591
P 3250 1100
F 0 "C?" H 3365 1146 50  0000 L CNN
F 1 "10u" H 3365 1055 50  0000 L CNN
F 2 "" H 3288 950 50  0001 C CNN
F 3 "~" H 3250 1100 50  0001 C CNN
	1    3250 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	3250 950  3250 900 
Wire Wire Line
	3250 900  3200 900 
Connection ~ 3250 900 
$Comp
L Device:C C?
U 1 1 5ECD44E1
P 3600 1100
F 0 "C?" H 3715 1146 50  0000 L CNN
F 1 "0.1u" H 3715 1055 50  0000 L CNN
F 2 "" H 3638 950 50  0001 C CNN
F 3 "~" H 3600 1100 50  0001 C CNN
	1    3600 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 950  3600 900 
Wire Wire Line
	3250 900  3600 900 
Wire Wire Line
	3600 1250 3600 1300
Wire Wire Line
	3250 1250 3250 1300
Wire Wire Line
	2450 1250 2450 1300
$Comp
L power:+5V #PWR?
U 1 1 5ECD63BB
P 3600 850
F 0 "#PWR?" H 3600 700 50  0001 C CNN
F 1 "+5V" H 3615 1023 50  0000 C CNN
F 2 "" H 3600 850 50  0001 C CNN
F 3 "" H 3600 850 50  0001 C CNN
	1    3600 850 
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 850  3600 900 
Connection ~ 3600 900 
Text Label 2350 900  0    50   ~ 0
VIN_FS
Wire Wire Line
	3600 900  3800 900 
$Comp
L Device:Ferrite_Bead FB?
U 1 1 5ECD7DC2
P 3950 900
F 0 "FB?" V 4224 900 50  0000 C CNN
F 1 "Ferrite_Bead" V 4133 900 50  0000 C CNN
F 2 "" V 3880 900 50  0001 C CNN
F 3 "~" H 3950 900 50  0001 C CNN
	1    3950 900 
	0    -1   -1   0   
$EndComp
$Comp
L Device:C C?
U 1 1 5ECD96A6
P 4150 1100
F 0 "C?" H 4265 1146 50  0000 L CNN
F 1 "0.1u" H 4265 1055 50  0000 L CNN
F 2 "" H 4188 950 50  0001 C CNN
F 3 "~" H 4150 1100 50  0001 C CNN
	1    4150 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 950  4150 900 
Wire Wire Line
	4150 900  4100 900 
Wire Wire Line
	4150 1250 4150 1300
Wire Wire Line
	2900 1200 2900 1300
Wire Wire Line
	4150 900  4550 900 
Connection ~ 4150 900 
Wire Wire Line
	5050 1300 5050 1200
$Comp
L Device:C C?
U 1 1 5ED4D72B
P 4550 1100
F 0 "C?" H 4665 1146 50  0000 L CNN
F 1 "4.7u" H 4665 1055 50  0000 L CNN
F 2 "" H 4588 950 50  0001 C CNN
F 3 "~" H 4550 1100 50  0001 C CNN
	1    4550 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	4550 950  4550 900 
Connection ~ 4550 900 
Wire Wire Line
	4550 900  4750 900 
Wire Wire Line
	4550 1250 4550 1300
$Comp
L Device:C C?
U 1 1 5ED4F6A6
P 5450 1100
F 0 "C?" H 5565 1146 50  0000 L CNN
F 1 "4.7u" H 5565 1055 50  0000 L CNN
F 2 "" H 5488 950 50  0001 C CNN
F 3 "~" H 5450 1100 50  0001 C CNN
	1    5450 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 950  5450 900 
Wire Wire Line
	5450 1250 5450 1300
$Comp
L power:GND #PWR?
U 1 1 5ED4F6B2
P 5450 1300
F 0 "#PWR?" H 5450 1050 50  0001 C CNN
F 1 "GND" H 5455 1127 50  0000 C CNN
F 2 "" H 5450 1300 50  0001 C CNN
F 3 "" H 5450 1300 50  0001 C CNN
	1    5450 1300
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 900  5350 900 
Wire Wire Line
	2900 1300 3250 1300
Connection ~ 5450 1300
Connection ~ 3250 1300
Wire Wire Line
	3250 1300 3600 1300
Connection ~ 3600 1300
Wire Wire Line
	3600 1300 4150 1300
Connection ~ 4150 1300
Wire Wire Line
	4150 1300 4550 1300
Connection ~ 4550 1300
Wire Wire Line
	4550 1300 5050 1300
Connection ~ 5050 1300
Wire Wire Line
	5050 1300 5450 1300
Wire Wire Line
	2900 1300 2450 1300
Connection ~ 2900 1300
Wire Wire Line
	5450 900  5450 850 
Connection ~ 5450 900 
Text Notes 3150 1450 0    50   ~ 0
1A MAX TOTAL ACROSS +5V AND +3.3V RAILS
Text Notes 750  1200 0    50   ~ 0
8-28V INPUT
Wire Wire Line
	9500 1750 9500 1800
Wire Wire Line
	9500 1800 9600 1800
Wire Wire Line
	9600 1800 9600 1850
Connection ~ 9500 1800
Wire Wire Line
	9500 1800 9500 1850
Wire Wire Line
	9600 1800 9700 1800
Connection ~ 9600 1800
Wire Wire Line
	9900 1800 9900 1850
Wire Wire Line
	9800 1800 9800 1850
Connection ~ 9800 1800
Wire Wire Line
	9800 1800 9900 1800
Wire Wire Line
	9700 1800 9700 1850
Connection ~ 9700 1800
Wire Wire Line
	9700 1800 9800 1800
$Comp
L Device:C C?
U 1 1 5ED616E7
P 8700 1100
F 0 "C?" H 8815 1146 50  0000 L CNN
F 1 "0.1u" H 8815 1055 50  0000 L CNN
F 2 "" H 8738 950 50  0001 C CNN
F 3 "~" H 8700 1100 50  0001 C CNN
	1    8700 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 900  8700 950 
Wire Wire Line
	8700 900  9100 900 
Wire Wire Line
	10800 900  10800 950 
Wire Wire Line
	10300 950  10300 900 
Wire Wire Line
	9900 950  9900 900 
Connection ~ 9900 900 
Wire Wire Line
	9900 900  10300 900 
Wire Wire Line
	9500 950  9500 900 
Connection ~ 9500 900 
Wire Wire Line
	9500 900  9900 900 
Wire Wire Line
	9100 950  9100 900 
Connection ~ 9100 900 
Wire Wire Line
	9100 900  9500 900 
Wire Wire Line
	8700 1250 8700 1300
Wire Wire Line
	8700 1300 9100 1300
Wire Wire Line
	10800 1300 10800 1250
Wire Wire Line
	10300 1300 10300 1250
Connection ~ 10300 1300
Wire Wire Line
	10300 1300 10800 1300
Wire Wire Line
	9900 1250 9900 1300
Connection ~ 9900 1300
Wire Wire Line
	9900 1300 10300 1300
Wire Wire Line
	9500 1250 9500 1300
Connection ~ 9500 1300
Wire Wire Line
	9500 1300 9900 1300
Wire Wire Line
	9100 1250 9100 1300
Connection ~ 9100 1300
Wire Wire Line
	9100 1300 9500 1300
Wire Wire Line
	10800 1300 10800 1350
Connection ~ 10800 1300
$Comp
L power:GND #PWR?
U 1 1 5ED72660
P 10800 1350
F 0 "#PWR?" H 10800 1100 50  0001 C CNN
F 1 "GND" H 10805 1177 50  0000 C CNN
F 2 "" H 10800 1350 50  0001 C CNN
F 3 "" H 10800 1350 50  0001 C CNN
	1    10800 1350
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 5ED734C5
P 8300 850
F 0 "#PWR?" H 8300 700 50  0001 C CNN
F 1 "+3V3" H 8315 1023 50  0000 C CNN
F 2 "" H 8300 850 50  0001 C CNN
F 3 "" H 8300 850 50  0001 C CNN
	1    8300 850 
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 850  8300 900 
Connection ~ 8700 900 
$Comp
L power:GND #PWR?
U 1 1 5ED752E0
P 9900 5550
F 0 "#PWR?" H 9900 5300 50  0001 C CNN
F 1 "GND" H 9905 5377 50  0000 C CNN
F 2 "" H 9900 5550 50  0001 C CNN
F 3 "" H 9900 5550 50  0001 C CNN
	1    9900 5550
	1    0    0    -1  
$EndComp
Wire Wire Line
	9900 5550 9900 5500
Wire Wire Line
	9900 5500 9800 5500
Wire Wire Line
	9500 5500 9500 5450
Connection ~ 9900 5500
Wire Wire Line
	9900 5500 9900 5450
Wire Wire Line
	9600 5450 9600 5500
Connection ~ 9600 5500
Wire Wire Line
	9600 5500 9500 5500
Wire Wire Line
	9700 5450 9700 5500
Connection ~ 9700 5500
Wire Wire Line
	9700 5500 9600 5500
Wire Wire Line
	9800 5450 9800 5500
Connection ~ 9800 5500
Wire Wire Line
	9800 5500 9700 5500
$Comp
L Device:C C?
U 1 1 5ED86BDC
P 9100 1100
F 0 "C?" H 9215 1146 50  0000 L CNN
F 1 "0.1u" H 9215 1055 50  0000 L CNN
F 2 "" H 9138 950 50  0001 C CNN
F 3 "~" H 9100 1100 50  0001 C CNN
	1    9100 1100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 5ED86FE7
P 9500 1100
F 0 "C?" H 9615 1146 50  0000 L CNN
F 1 "0.1u" H 9615 1055 50  0000 L CNN
F 2 "" H 9538 950 50  0001 C CNN
F 3 "~" H 9500 1100 50  0001 C CNN
	1    9500 1100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 5ED8744A
P 9900 1100
F 0 "C?" H 10015 1146 50  0000 L CNN
F 1 "0.1u" H 10015 1055 50  0000 L CNN
F 2 "" H 9938 950 50  0001 C CNN
F 3 "~" H 9900 1100 50  0001 C CNN
	1    9900 1100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 5ED879E3
P 10300 1100
F 0 "C?" H 10415 1146 50  0000 L CNN
F 1 "0.1u" H 10415 1055 50  0000 L CNN
F 2 "" H 10338 950 50  0001 C CNN
F 3 "~" H 10300 1100 50  0001 C CNN
	1    10300 1100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 5ED87F52
P 10800 1100
F 0 "C?" H 10915 1146 50  0000 L CNN
F 1 "0.1u" H 10915 1055 50  0000 L CNN
F 2 "" H 10838 950 50  0001 C CNN
F 3 "~" H 10800 1100 50  0001 C CNN
	1    10800 1100
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 5ED93EE4
P 8300 1100
F 0 "C?" H 8415 1146 50  0000 L CNN
F 1 "10u" H 8415 1055 50  0000 L CNN
F 2 "" H 8338 950 50  0001 C CNN
F 3 "~" H 8300 1100 50  0001 C CNN
	1    8300 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 1250 8300 1300
Wire Wire Line
	8300 1300 8700 1300
Connection ~ 8700 1300
Wire Wire Line
	8300 950  8300 900 
Wire Wire Line
	8300 900  8700 900 
Connection ~ 8300 900 
$Comp
L Device:C C?
U 1 1 5ED9A64E
P 8650 2250
F 0 "C?" H 8765 2296 50  0000 L CNN
F 1 "0.1u" H 8765 2205 50  0000 L CNN
F 2 "" H 8688 2100 50  0001 C CNN
F 3 "~" H 8650 2250 50  0001 C CNN
	1    8650 2250
	1    0    0    -1  
$EndComp
Wire Wire Line
	8650 2100 8650 2050
Wire Wire Line
	8650 2050 9000 2050
$Comp
L power:GND #PWR?
U 1 1 5ED9D813
P 8650 2450
F 0 "#PWR?" H 8650 2200 50  0001 C CNN
F 1 "GND" H 8655 2277 50  0000 C CNN
F 2 "" H 8650 2450 50  0001 C CNN
F 3 "" H 8650 2450 50  0001 C CNN
	1    8650 2450
	1    0    0    -1  
$EndComp
Wire Wire Line
	8650 2450 8650 2400
Text Notes 8950 2300 0    50   ~ 0
?
$Comp
L Device:Ferrite_Bead FB?
U 1 1 5EDA2FAE
P 10550 900
F 0 "FB?" V 10750 900 50  0000 C CNN
F 1 "Ferrite_Bead" V 10700 900 50  0000 C CNN
F 2 "" V 10480 900 50  0001 C CNN
F 3 "~" H 10550 900 50  0001 C CNN
	1    10550 900 
	0    -1   -1   0   
$EndComp
Connection ~ 10300 900 
Wire Wire Line
	10300 900  10400 900 
Wire Wire Line
	10800 900  10700 900 
Wire Wire Line
	10800 900  10800 850 
Connection ~ 10800 900 
$Comp
L power:+3.3VA #PWR?
U 1 1 5EDB5350
P 10800 850
F 0 "#PWR?" H 10800 700 50  0001 C CNN
F 1 "+3.3VA" H 10815 1023 50  0000 C CNN
F 2 "" H 10800 850 50  0001 C CNN
F 3 "" H 10800 850 50  0001 C CNN
	1    10800 850 
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3VA #PWR?
U 1 1 5EDC980F
P 10000 1750
F 0 "#PWR?" H 10000 1600 50  0001 C CNN
F 1 "+3.3VA" H 10015 1923 50  0000 C CNN
F 2 "" H 10000 1750 50  0001 C CNN
F 3 "" H 10000 1750 50  0001 C CNN
	1    10000 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	10000 1750 10000 1850
$Comp
L power:+3.3V #PWR?
U 1 1 5EDCC459
P 5450 850
F 0 "#PWR?" H 5450 700 50  0001 C CNN
F 1 "+3.3V" H 5465 1023 50  0000 C CNN
F 2 "" H 5450 850 50  0001 C CNN
F 3 "" H 5450 850 50  0001 C CNN
	1    5450 850 
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR?
U 1 1 5EDCCE36
P 9500 1750
F 0 "#PWR?" H 9500 1600 50  0001 C CNN
F 1 "+3.3V" H 9515 1923 50  0000 C CNN
F 2 "" H 9500 1750 50  0001 C CNN
F 3 "" H 9500 1750 50  0001 C CNN
	1    9500 1750
	1    0    0    -1  
$EndComp
$Comp
L Device:LED_CRGB D?
U 1 1 5ED607D0
P 2250 2250
F 0 "D?" H 2250 1783 50  0000 C CNN
F 1 "LED_CRGB" H 2250 1874 50  0000 C CNN
F 2 "" H 2250 2200 50  0001 C CNN
F 3 "~" H 2250 2200 50  0001 C CNN
	1    2250 2250
	-1   0    0    1   
$EndComp
$Comp
L Device:LED_CRGB D?
U 1 1 5ED63AFB
P 2250 3100
F 0 "D?" H 2250 2633 50  0000 C CNN
F 1 "LED_CRGB" H 2250 2724 50  0000 C CNN
F 2 "" H 2250 3050 50  0001 C CNN
F 3 "~" H 2250 3050 50  0001 C CNN
	1    2250 3100
	-1   0    0    1   
$EndComp
$Comp
L Device:R_US R?
U 1 1 5ED667AA
P 1850 2050
F 0 "R?" V 1900 2200 50  0000 C CNN
F 1 "R_US" V 1900 1900 50  0000 C CNN
F 2 "" V 1890 2040 50  0001 C CNN
F 3 "~" H 1850 2050 50  0001 C CNN
	1    1850 2050
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_US R?
U 1 1 5ED67CEC
P 1850 2250
F 0 "R?" V 1900 2400 50  0000 C CNN
F 1 "R_US" V 1900 2100 50  0000 C CNN
F 2 "" V 1890 2240 50  0001 C CNN
F 3 "~" H 1850 2250 50  0001 C CNN
	1    1850 2250
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_US R?
U 1 1 5ED6846E
P 1850 2450
F 0 "R?" V 1900 2600 50  0000 C CNN
F 1 "R_US" V 1900 2300 50  0000 C CNN
F 2 "" V 1890 2440 50  0001 C CNN
F 3 "~" H 1850 2450 50  0001 C CNN
	1    1850 2450
	0    -1   -1   0   
$EndComp
Wire Wire Line
	2000 2050 2050 2050
Wire Wire Line
	2000 2250 2050 2250
Wire Wire Line
	2000 2450 2050 2450
$Comp
L Device:R_US R?
U 1 1 5ED70251
P 1850 2900
F 0 "R?" V 1900 3050 50  0000 C CNN
F 1 "R_US" V 1900 2750 50  0000 C CNN
F 2 "" V 1890 2890 50  0001 C CNN
F 3 "~" H 1850 2900 50  0001 C CNN
	1    1850 2900
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_US R?
U 1 1 5ED7025B
P 1850 3100
F 0 "R?" V 1900 3250 50  0000 C CNN
F 1 "R_US" V 1900 2950 50  0000 C CNN
F 2 "" V 1890 3090 50  0001 C CNN
F 3 "~" H 1850 3100 50  0001 C CNN
	1    1850 3100
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_US R?
U 1 1 5ED70265
P 1850 3300
F 0 "R?" V 1900 3450 50  0000 C CNN
F 1 "R_US" V 1900 3150 50  0000 C CNN
F 2 "" V 1890 3290 50  0001 C CNN
F 3 "~" H 1850 3300 50  0001 C CNN
	1    1850 3300
	0    -1   -1   0   
$EndComp
Wire Wire Line
	2000 2900 2050 2900
Wire Wire Line
	2000 3100 2050 3100
Wire Wire Line
	2000 3300 2050 3300
Wire Wire Line
	2450 3100 2500 3100
Wire Wire Line
	2500 3100 2500 3150
Wire Wire Line
	2450 2250 2500 2250
Wire Wire Line
	2500 2250 2500 2300
$Comp
L power:GND #PWR?
U 1 1 5ED7AAE8
P 2500 2300
F 0 "#PWR?" H 2500 2050 50  0001 C CNN
F 1 "GND" H 2505 2127 50  0000 C CNN
F 2 "" H 2500 2300 50  0001 C CNN
F 3 "" H 2500 2300 50  0001 C CNN
	1    2500 2300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5ED7B703
P 2500 3150
F 0 "#PWR?" H 2500 2900 50  0001 C CNN
F 1 "GND" H 2505 2977 50  0000 C CNN
F 2 "" H 2500 3150 50  0001 C CNN
F 3 "" H 2500 3150 50  0001 C CNN
	1    2500 3150
	1    0    0    -1  
$EndComp
$Comp
L Hyperlib:AP2114H-3.3TRG1 U?
U 1 1 5EDF2120
P 5050 950
F 0 "U?" H 5050 1265 50  0000 C CNN
F 1 "AP2114H-3.3TRG1" H 5050 1174 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 5000 1000 50  0001 C CNN
F 3 "https://www.diodes.com/assets/Datasheets/AP2114.pdf" H 5000 1000 50  0001 C CNN
	1    5050 950 
	1    0    0    -1  
$EndComp
$EndSCHEMATC
